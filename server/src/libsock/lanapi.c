...
SMSHead    *chead1;
	...
	/*-----------------------------------------------------------*/
    if (SMS_Server == ON) {
		userlng = UINT(chead1->Lng [0],chead1->Lng [1],chead1->Lng [2],chead1->Lng [3]);
	}
	else {
    	/*userlng = USHORT (chead->Lng [0], chead->Lng [1]);*/
...
struct sockaddr_in  serv_addr, SockAddr;
    ...
    return (sockfd);
    /*
    if (ConnectWait(sockfd,(struct sockaddr*)&serv_addr,sizeof(serv_addr), 2) < 0) {
    	close (sockfd);
        return (-1);
    }
    else{
