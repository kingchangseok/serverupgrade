...
char	szArgVal                   [100];
	...
	/*-----------------------------------------------------------*/
	/* ecams_svr PORT=29895 LOG=TRUE LOGDIR=AgentLog BUFFER=10240000 */
	/*-----------------------------------------------------------*/
	for (i = 1; i < argc; i++) {
		sprintf(szArgv, "%s", argv[i]);
		nRet = Char_Check(szArgv, "=");
		if (nRet >= 0) {
			sprintf(szArgKey, "%s", left_char (szArgv, nRet));
			sprintf(szArgVal, "%s", right_char(szArgv, strlen(szArgv) - nRet - 1));
			sprintf(szArgKey2, "%s", upper_char (szArgKey));
			sprintf(szArgKey, "%s",szArgKey2);
			if (strcmp(szArgKey, "PORT") == 0) {
				dfSendPort = atoi(szArgVal);
			}

			else
			if (strcmp(szArgKey, "LOG") == 0) {
				sprintf(szArgVal, "%s", upper_char (szArgVal));				
				if (strcmp(szArgVal, "TRUE") == 0) {
					LogFg = 1;
				}
			}

			else
			if (strcmp(szArgKey, "LOGDIR") == 0) {
				sprintf(LogDir, "%s", szArgVal);
			}

			else
			if (strcmp(szArgKey, "BUFFER") == 0) {
				if (atoi(szArgVal) > 0) {
					dfMaxFileSize = atoi(szArgVal);
				}
			}
		}
	}
...
int		i;
    ...
    if (pulng%16 == 0) {
		...
		nCmdSize = (pulng/16+1) * 16;
	}
	
    /*-----------------------------------------------------------*/
    /* 	수행할 System 명령을 Get.                                */
    /*-----------------------------------------------------------*/
    tmpbuf = (char *) malloc (dfMaxBufSize);
    memset (tmpbuf, 0x00, dfMaxBufSize);
    memcpy (tmpbuf, &RdBuf[hdlng], pulng);
	decbuf = (char *) malloc (dfMaxBufSize);	
	memset (decbuf, 0x00, dfMaxBufSize);
    ...
    memset (tmpbuf, 0x00, dfMaxFileSize);
    if (!retval) {                                /* 성공한 경우 */
