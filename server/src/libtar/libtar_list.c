...
libtar_list_add(libtar_list_t *l, void *data)
{
	...
	datatmp = (char *) malloc (sizeof(char)*512);

	n = (libtar_listptr_t)malloc(sizeof(struct libtar_node));

	strcpy(datatmp,(char*)data);

	if (n == NULL)
		return -1;

	memset(n,0x00,sizeof (struct libtar_node));
	...
	if (l->flags == LIST_QUEUE)
	{
		...
		printf("<== libtar_list_add(): LIST_QUEUE set; "
		       "added at end prev->data[%s] data[%s]\n",(char *)l->last->prev->data,(char*)l->last->data);
#endif
		return 0;
	}
