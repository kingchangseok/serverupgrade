...
tar_extract_regfile(TAR *t, char *realname)
{
...
#ifdef DEBUG
	printf("==> tar_extract_regfile(t=0x%lx, realname=\"%s\" filename=\"%s\")\n", t,
	       realname,filename);
#endif
	memset(dirnametmp,0x00,sizeof(dirnametmp));
#ifdef DEBUG
	printf("==> tar_extract_regfile1(t=0x%lx, realname=\"%s\" filename=\"%s\")\n", t,
	       realname,filename);
#endif
	strcpy(dirnametmp,filename);
#ifdef DEBUG
	printf("==> tar_extract_regfile2(t=0x%lx, realname=\"%s\" filename=\"%s\")\n", t,
	       realname,filename);
#endif

	if (mkdirhier(dirname(dirnametmp)) == -1)
		return -1;
	...
	printf("  ==> extracting: %s (mode %04o, uid %d, gid %d, %d bytes)\n",
	       filename, mode, uid, gid, (int)size);
#endif
